...
BEGIN
    ...
    SELECT @salt = salt, @oldPasswordHash = passwordHash, @oldFirstName = firstName, @oldLastName = lastName, @oldActive = active, @oldComment = comment FROM [dbo].[Users] WHERE id = @pId
    ...
    BEGIN TRY
        ...
        UPDATE Users SET passwordHash = @newPasswordHash, firstName = @pFirstName, lastName = @pLastName, active = @pActive, comment = @pComment WHERE id = @pId
